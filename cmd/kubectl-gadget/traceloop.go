...
func runTraceloopList(cmd *cobra.Command, args []string) {
	...
	for _, trace := range traces {
		...
		case "created":
			fallthrough
		case "ready":
			if trace.TimeCreation != "" {
				t, err := time.Parse(time.RFC3339, trace.TimeCreation)
				if err == nil {
					status = fmt.Sprintf("started %s ago",
						strings.ToLower(units.HumanDuration(time.Now().Sub(t))))
				} else {
					status = fmt.Sprintf("started a while ago (%v)", err)
				}
			} else {
				status = "started"
			}
		case "deleted":
			if trace.TimeDeletion != "" {
				t, err := time.Parse(time.RFC3339, trace.TimeDeletion)
				if err == nil {
					status = fmt.Sprintf("terminated %s ago",
						strings.ToLower(units.HumanDuration(time.Now().Sub(t))))
				} else {
					status = fmt.Sprintf("terminated a while ago (%v)", err)
				}
			} else {
				status = "terminated"
			}
		...
		if optionListFull {
			fmt.Fprintf(w, "%v\t%v\t%v\t%v\t%v\t%v\t%v\t%v\t%v\n", trace.Node, trace.Namespace, trace.Podname, trace.UID, trace.Containeridx, trace.TraceID, trace.ContainerID, status, capDecode(trace.Capabilities))
		} else {
			uid := trace.UID
			if len(uid) > 8 {
				uid = uid[:8]
